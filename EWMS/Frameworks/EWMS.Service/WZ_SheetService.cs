namespace EWMS.Service
{
    ...
    public class WZ_SheetService : BaseService<WZ_Sheet>
    {
        private static readonly WZ_SheetService _Instance = new WZ_SheetService();

        public int InsertOtherSheet(WZ_Sheet sheet, int creator, string kindcode, Base_Menu menu)
        {
            try
            {
                string str = "WZSHEET_SEQUENCE";
        ...
        public int InsertSheet(WZ_Sheet sheetdata, int userid, string kindcode, string menucode, string jumppath)
        {
            string str = "WZSHEET_SEQUENCE";
            dynamic obj2 = new ExpandoObject();
            obj2.Code = "seqName";
           ...
           dictionary.Add(obj2, str);
            ...
            dictionary2.Add(obj4, "");
            OracleParameter[] parameterArray = base.SP_OtherFish("GetIDs", dictionary, dictionary2);
            sheetdata.ID = Convert.ToInt32(parameterArray[0].Value.ToString());
            string str2 = parameterArray[1].Value.ToString();
            sheetdata.GUID = Guid.NewGuid().ToString();
            sheetdata.SubmitManId = userid;
            sheetdata.Creator = userid;
            sheetdata.SubmitTime = DateTime.Now;
            sheetdata.CreateDate = DateTime.Now;
            sheetdata.ZTID = 0x3e8;
            int num = 0;
            try
            {
                num = Instance.Insert(sheetdata);
            }
            catch (Exception)
            {
                return -2;
            ...
            if (num > 0)
            {
                ...
                dictionary3.Add(obj6, "0");
                dictionary3.Add(obj7, str2.ToString());
                dictionary3.Add(obj8, menucode);
                dictionary3.Add(obj9, userid.ToString());
                dictionary3.Add(obj10, jumppath);
                dictionary3.Add(obj11, "1000");
                Dictionary<object, string> dictionary4 = new Dictionary<object, string>();
                dictionary4.Add(obj12, "");
                dictionary4.Add(obj13, "");
        ...
        public int UpdateOtherSheet(ParamUpdate param)
        {
            try
            {
        ...
        public int UpdateSheet(ParamUpdate param)
        {
            try
            {
                return Instance.Update(param);
        ...
        public static WZ_SheetService Instance
        {
            get
            {
                return _Instance;
