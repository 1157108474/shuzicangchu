layui.use(['form', 'layer', 'table'], function () {
    var form = layui.form,
        ...
        $ = layui.$,
        table = layui.table;
    form.on("submit(updateApplyDep)", function (pvi) {
        ...
        if(selects == ""){
            ...
            $.ajax({
                type: "POST",
                url: "/system/applyDep/editApplyDep.json",
                dataType: "json",
                data: JSON.parse(JSON.stringify(pvi.field)),
                success: function (ret) {
                    if (ret.status == 1) {
                        layer.msg(ret.message, function () {
                            parent.layui.table.reload("applyDepListReload");
...
function obtainPart(id,name,ztid){
