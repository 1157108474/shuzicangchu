    ...
    var detailsgrid = table.render({
        ...
        , cols: [
            [{type: "checkbox", fixed: "left", width: 30}
                ...
                , {field: 'storeLocationCode', title: '调入库位', align: "center",width: 140}
                , {field: 'detailUnitName', title: '单位', align: "center",width: 60}
                , {field: 'providerName', title: '拥有方', align: "center",width: 240}
                , {field: 'detailCount', title: '计划调拨数量', align: "center", width: 120}
                , {field: 'ytCount', title: '实际调拨数量', align: "center", width: 120}
            ]
        ]
    });
...
//保存 按钮
    ...
    var open = function () {   //新增明细页面
        ...
        layer.open({
            ...
            , fixed: false
            , area: ["90%", "90%"]
            , content: "/sheet/db/openAddDetail"
            , end: function () {
    ...
    var saveSheet =function(openFlag){
        ...
        var cOrg = $("#cOrg").val();
        if(cOrg==""){
            layer.alert("请选择调出库存组织");
            return ;
        }
        if(rOrg==""){
            layer.alert("请选择调入库存组织");
            return ;
        }
        if(rOrg == cOrg){
            layer.alert("调入库存不得与调出库存一致");
            return ;
        }
        $.ajax({
            type: "post",
            url: "/sheet/WZDBD",
            dataType: "json",
            data: {"memo": $("#memo").val(), "menuCode": $("#menuCode").val(),extendInt1:cOrg,extendInt2:rOrg,"createDate":$("#createDate").val()},
    ...
    $("#deleteDetails").on("click", function (e) {
        ...
        if (data.length > 0) {
            ...
            layer.msg('请选择一条数据', {offset: 't', anim: 6});
        }
    });
