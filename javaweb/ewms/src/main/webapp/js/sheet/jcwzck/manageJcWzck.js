layui.config({
    ...
    var ordergr = table.render({
        ...
        , cols: [
            [{type: 'numbers', title: '序号',fixed: "left", width: 50}
                ...
                , {field: 'ownerDepName', title: '所属单位', align: "center"}
                , {field: 'useName', title: '领用人', align: "center"}
    ...
    //监听工具条
    table.on('tool(jcwzckgrid)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            parent.tab.tabAdd({
                href: data.url+"?oper=edit", //地址
                title: "编辑寄存物资出库单"+data.code
            });
        }else if(obj.event === 'show'){
            parent.tab.tabAdd({
                href: data.url+"?oper=show", //地址
                title:"查看寄存物资出库单"+data.code
            });
        }else if(obj.event === 'delete'){
            layer.confirm('确定删除选中的记录？', {icon: 3, title: '提示信息'}, function (index) {
                $.ajax({
                    type: "delete",
                    url: "/sheet/wzjcckd-"+data.id,
                    dataType: "json",
